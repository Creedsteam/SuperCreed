...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return '<b>Lock Link was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '<b>Lock Spam[long msg] was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function lock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == 'yes' then
    return 'SuperGroup Emoji is already locked'
  else
...
local function lock_group_badword(msg, data, target)
  ...
  if group_badword_lock == 'yes' then
    return 'SuperGroup badword is already locked'
  else
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return '<b>Lock flood was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return '<b>Lock Arabic was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'no' then
    return '<b>Lock Tag[ @ ] was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_hashtag(msg, data, target)
  ...
  if group_hashtag_lock == 'no' then
    return '<b>Lock HashTag[ # ] was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'no' then
    return '<b>Lock Forward was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return '<b>Lock Member join was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return '<b>Lock RTL[Right To left Mesages] was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return '<b>Lock TGservice[Join Group Pm] was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return '<b>Lock Sticker was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'no' then
    return '<b>Lock Reply was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return '<b>Lock Contacts was on</b> : <code>unlock</code>\n<b>Status not changed !</b>'
  else
...
function show_supergroup_settingsmod(msg, target)
  ...
  .."<i>Lock Reply</i> : "..settings.lock_reply.."\n"
  .."<i>Lock Contact share</i> : "..settings.lock_contacts.."\n"
  .."<i>Lock links</i> : "..settings.lock_link.."\n"
  .."<i>Lock Forward</i> : "..settings.lock_fwd.."\n"
.."<i>Lock #hashtag</i> : "..settings.lock_hashtag.."\n"
.."<i>Lock tag</i> : "..settings.lock_tag.."\n"
.."<i>Lock sticker</i> : "..settings.lock_sticker.."\n"
.."<i>Lock flood</i> : "..settings.flood.."\n"
.."<i>Lock spam</i> : "..settings.lock_spam.."\n"
.."<i>Lock Badwords</i> : "..settings.lock_badword.."\n"
.."<i>Lock Emoji</i> : "..settings.lock_emoji.."\n"
...
.."<i>Flood number</i> : "..NUM_MSG_MAX.."\n"
.."<i>Public SuperGroup</i>: "..settings.public.."\n"
.."<i>Group type</i> : SuperGroup\n"
.."<i>Bot Version</i> : 2.0\n"
.."<i>Channel</i> : @IR_TEAM\n"
.."<i>Strict settings</i>: "..settings.strict.."\n\n"
.."<b>Requester Information</b> :\n"
.."<code>Name</code> : "..msg.from.first_name.." "..(msg.from.last_name or '').."\n"
...
local bot_id = 214035529
local rlm = 1051763059
...
if matches[1]:lower() == 'setexpire' then
    ...
    return reply_msg(msg.id, "<b>Group charged for</b><code> "..matches[2].." </code><b>Days</b>", ok_cb, false)
  end
	...
	local text_add = '<b>New Group Added</b> :\n<b>Gp Info</b> :\n<b>Gp ID </b>: <code>'..msg.to.id..'</code>\n<b>Gp name</b> : <b>'..msg.to.title..'</b>\n<b>Adder Info</b>\n<b>User ID</b> : <code>'..msg.from.id..'</code>\n<b>Username </b>: '..(msg.from.username or '')..'\n<b>You can leave the bot by</b> /leave'..msg.to.id..' <b>Command</b>\n<b>You can join it by</b> : /join'..msg.to.id..' <b>command</b>\n<b>To charge it for one month use</b> : /1m'..msg.to.id..'\nDone !'
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "<b>Group id</b> : <code>"..msg.to.id.."</code>\n<b>Your id</b> : <code>"..msg.from.id.."</code>\n<b>Your Name</b> : <code>"..msg.from.first_name.."</code>"
			end
		end
        ...
        local data_cat = 'banlist'
           ...
           save_data(_config.moderation.data, data)
           send_large_msg(get_receiver(msg), "ban list Cleare now")
           redis:del(hash)
     end
			if matches[2] == 'gbl' and is_sudo(msg) then 
        local hash = 'gbanned'
        local data_cat = 'gbanlist'
           data[tostring(msg.to.id)][data_cat] = nil
           save_data(_config.moderation.data, data)
           send_large_msg(get_receiver(msg), "Gban list Cleare now")
           redis:del(hash)
     end

...
if to == 'user' or service or is_admin1(msg) and to == "chat" or to == "channel" then
	...
	elseif matches[2]:lower() == 'persian' and matches[3]:lower() == 'support' then
		...
		if data[tostring(target)]['settings']['lock_member'] == 'yes' and not is_owner2(msg.from.id, '1062007190') then
			return 'Group is private.'
		end
		if is_admin1(msg) then
			user_type = 'admin'
	...
	if matches[1] == 'help' and msg.to.type == 'user' or matches[1] == 'pmhelp' and is_admin1(msg) and msg.to.type ~= 'user' then
		...
		text = "Welcome to my Private!\nFirst <code>SHARE YOUR CONTACT</code>\n\nJoin our support : https://telegram.me/joinchat/DMHsST9M8ZZ2I6rD2uEWCQ\nYou can use these commands to have fun !\n🔰!loc place,adress\nبرای دیدن آدرس یه محل مثل ادرس یه خیابون یه شهر روی نقشه\n🔰!time country\nبرای دیدن تایم یه شهر\n🔰!write text\nبرای دریافت نوشته مورد نظر شما در فونت های مختلف\n🔰/photo\nبا ریپلی کردن زیر استیکر به صورت عکس دریافتش کنید\n🔰/sticker\nبا ریپلی زیر عکس استیکرشو بگیرید /\n🔰!qr text\nبرای تبدیل متن به عکس بار کدی . \n🔰!calc (mathmatic)\nبرای محاسبه اعمال اصلی ریاضی \n🔰!music (esm musigi)\n🔰!dl (addad musigi mored nazar az list)\nبرای دریافت لیست آهنگ های خواننده خواسته شده  و برای دانلود کردن ترانه با عدد خودش از لیست\n\n\n🔰!porn (name)\nبرای دریافت لینک پورن جست و جو شده از 5 تا سایت پورن و سکسی\n🔰!(text) print\nبرای تبدیل نوشته شما به عکس\n🔰!tosticker (Size) (text)\nبرای تبدیل متن شما در سایز مورد نظر که باید عدد باشه به استیکر\n\n🔰!weather (City or loc)\nبرای دیدن وضعیت آب و هوایی یک شهر یا منطقه\n🔰!voice (text)\nبرای تبدیل متن به صوت \n🔰!boobs \nبرای دیدن عکس ممه\n\n🔰!butts\nبرای دیدن عکس کون \n🔰!gif (text)\nدرباره ی کلمه مورد نظر گیف میفرسته . \n\n🔰!giphy\nیه گیف تصادفی میفرسته ! \n🔰!wiki search (text)\nهم خانواده های کلمه سرچ شده رو میده \n\n🔰!wiki (text)\nدرباره کلمه مورد نظر سرچ میکنه مطلب میده \n\n🔰!time\nبرای دریافت زمان الان کشورمان به صورت استیکر و پشت زمینه های زیبا.\n🔰!keep calm and text text\nبا جایگزاری فقط دو کلمه به جای text استیکر keep calm را دریافت کنید"
     	return text
    end

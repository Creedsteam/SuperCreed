...
local function convert(text)
    ...
    local text = text:gsub('ئ','ي')
    local text = text:gsub('آ','ا')
    local text = text:gsub('?','ه')
    local text = text:gsub('ي','ي')
...
local function run(msg, matches)
       if not matches[2] or matches[2] == '' then
        return [[حروف جمل يا به عبارت ديگر حروف ابجد،نام مجموع صور هشتگانه حروف عرب است. اين صور ازين قرار است: ابجد – هوز- حطي - کلمن - سعفص - قرشت - ثخذ - ضظغ.
ترتيب حروف (مراد،حروف صامت است) درين نسق همان ترتيب عبري آرامي است و اين امر با دلايل ديگر مؤيد آنست که عرب الفباي خود را از آنان بوساطت نبطيان اقتباس کرده و شش حرف مخصوص عرب در آخر ترتيب ابجدي قرار داده شده است؛ علاوه برين ترتيب هشت کلمه تذکاريه که مفهومي ندارند با عبري و آرامي در اينکه حروف معرف اعدادند نيز شباهت دارد،از «همزه» تا «ي» نماينده ي 1تا10 ،«ک» تا «ق» نماينده ي 20تا100 و نه حرف آخر معرف 200تا1000 باشد. ابجد تجريد نوشتن (تصوف) ترک خواهش و آرزو کردن و از خودي و مزاحمت خواهش آمدن و از ماسوي الله مجرد گرديدن...
ا=1     ک=20     ش=300
ب=2     ل=30     ت=400
ج=3     م=40     ث=500
د=4     ن=50     خ=600
...
if text ~= '' then
  return 'Only Persian/arabic word Supported !'  
end
return ' Number of  Your Arabic Code name : <code>['..num..']</code>'
    end
  return {
patterns = {
    "^[!/]([Aa]bjad) (.*)$",
